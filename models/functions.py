...
def segmentDataHandler(path,savefig,logger,rc,dv,logf):
    remove_thresh = 0.25
    ...
    for file in tqdm(fileList, desc=loop_desc):
        ...
        try:
            mac_si, mac_ei = getSegmentedDataPeaks(data_inst,dv)
            rc.purge_everything()
...
def readOneFile(filepath):
    import pandas as pd
    columns = ['time','fx','fy','fz','ax','ay','az','vv']
    data_inst = pd.read_csv(filepath, header=None, names=columns,sep='\t')
    return data_inst
